...
$count_rows='0';
if(!empty($_GET['dir_id']) && $_GET['dir_id']>0){
$count_rows= count_rows($table='tbl_uploads',$field='',$cond="where userId=$userId and file_dir_id='".$_GET['dir_id']."'");
}
...
<?php if($count_rows>0){?>
<h4 align="center">You can view <strong><?php  $table_rows=get_table_row($table='tbl_directory',$field='',$cond="where dir_id='".$_GET['dir_id']."' and dir_userId='$userId'");
echo $table_rows['dir_name'];
$dir=$base_path.'/'.$directory_name.'/'.$table_rows['dir_name'];
?></strong> folders / files List. <?php echo $table_rows['dir_name'];?> contains 
[ <?php echo $count_rows;?> ]</h4>
<?php
}
?>
 <form name="view" id="view" method="post" action="file_list.php" enctype="multipart/form-data" onsubmit="return validate(this)">
    ...
    <tr>
      ...
      <td><a class="btn btn-primary" href="create_directory.php" role="button">Create New Folder</a></td>
      <td>&nbsp;</td>
	...
	$sql="SELECT * FROM tbl_uploads where userId=$userId and file_dir_id='".$_GET['dir_id']."'";
	$result_set=mysql_query($sql) or die($sql.' '.mysql_error());;
	if(mysql_num_rows($result_set)>0){
	while($row=mysql_fetch_array($result_set))
	{
	  ...
	  if(mysql_num_rows($rsQuery)>0){
		...
		<td><?php 
		if($results['dir_name']==$userRow['userFoldername'])
		  echo 'root';
		else
	...
	?>
	 <tr>
        <td colspan="9"><a href="uploads/<?php echo $row['file'] ?>"target="_blank"></a><a href="canview.php" target="_blank"></a><a href="canedit.php" target="_blank">There is no record .!!!!!!!!!!!!! </a></td>
      </tr>
	<?php
	}
	?>
